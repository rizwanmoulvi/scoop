...
export class ProbableAdapter implements PredictionPlatform {
  ...
  buildOrder(params: TradeInput): Order {
    ...
    return {
      ...
      extra: {
        ...
        nonce:         '0',
        signatureType: 0,    // EOA direct — maker == signer == EOA (signatureType=1 broken: factory missing safeAddress() selector)
  ...
  async submitOrder(order: SignedOrder, _signer: WalletSigner): Promise<ApiResponse> {
    ...
    const requestBody = {
      deferExec: false, // EOA CLOB: place in off-chain orderbook; true = immediate on-chain (PAS-4205 if no match)
      order: {
      ...
      const data = await apiFetch<Record<string, unknown>>(
        `${this.orderbookApiBase}/order/${BSC_CHAIN_ID}`,
        {
          ...
          headers: {
            ...
            prob_account_type: 'eoa',  // required for signatureType=0 orders
          },
          body: bodyString,
        }
      )
