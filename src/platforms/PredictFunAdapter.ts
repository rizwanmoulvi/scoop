 ...
 * Predict.fun adapter.
 *
 * Predict.fun uses Gnosis Conditional Tokens with a CLOB orderbook.
 * Docs: https://docs.predict.fun
 *
 * TODO: Inspect network calls at https://app.predict.fun and update:
 *  - API base URL
 *  - EIP-712 domain / types
 *  - Order payload shape
 */
export class PredictFunAdapter implements PredictionPlatform {
  readonly name = 'Predict.fun'

  private readonly baseUrl = 'https://app.predict.fun/api'

  private get domain() {
    return {
      name: 'PredictFun',
      version: '1',
      chainId: 137, // Polygon mainnet (Gnosis CT common chain)
  ...
  async getMarket(id: string): Promise<Market> {
    ...
    if (!res.ok) throw new Error(`PredictFun: failed to fetch market ${id}: ${res.status}`)
    ...
    return {
      ...
      status: data.active ? 'open' : 'closed',
      resolutionDate: data.endDate,
      url: `https://app.predict.fun/markets/${id}`,
    }
  }

  async getOrderBook(id: string): Promise<OrderBook> {
    const res = await fetch(`${this.baseUrl}/markets/${id}/orderbook`)
    if (!res.ok) throw new Error(`PredictFun: failed to fetch orderbook for ${id}: ${res.status}`)
    const data = await res.json()
